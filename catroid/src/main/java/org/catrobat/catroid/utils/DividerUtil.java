...
public final class DividerUtil {
    ...
    public static void setDivider(Context context, BrickLayout brickLayout) {
        if (isActivated()) {
            //TODO
        }
    }
    ...
    public static void setDivider(Context context, ViewGroup viewGroup) {
        if (isActivated()) {
            Log.d("blah", "viewGroup sadsa");
            for(int i = 0; i < viewGroup.getChildCount(); i++) {
                Log.d("blah", "viewGroup: " + viewGroup.getChildAt(i).toString());
                if(viewGroup.getChildAt(i) instanceof BrickLayout) {
                    setDivider(context, (BrickLayout) viewGroup.getChildAt(i));
                }
            }
        }
    }
